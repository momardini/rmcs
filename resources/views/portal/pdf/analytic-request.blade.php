...
<div id="page-border">
    ...
    <table style="width: 100%;" class="noborderdtable">
        ...
        <tr>
            ...
            <td style="text-align: right;font-family: 'cairoregular';">
                ...
                <strong>العيادة : <span style="color: #9f3629;">{{$appointment->clinic->getTranslatedAttribute('title')}}</span></strong>
                <p style="text-align: right;" >رقم المعاينة : {{$appointment->id}}</p>
            </td>
        </tr>
        <tr>
            <td colspan="3" style="text-align: center;">
                <h2><strong>{{ __('portal.patient.analytics') }}</strong></h2>
            </td>
        </tr>
        <tr>
    ...
    <table style="width: 100%; margin-left: auto; margin-right: auto;" class="borderdtable">
        <tbody>
    @foreach(array_chunk($analytics->toArray(), 6) as $i=>$set)
        <tr>
            @foreach($set as $j=>$analytic)

                    @if ($interview->analytics->contains('id',$analytic['id']))
                        <td style="background: lightgrey;">
                        <span>
                            <strong>{{$analytic['title']}}</strong>
                            <img src="{{ url('/storage/resource/check.png') }}" width="10px"
                                 style="padding: 0px;margin: 0px;padding-left: 4px">
                        </span>
                        </td>
                    @else
